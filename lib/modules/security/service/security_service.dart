...
class SecurityService {
  ...
  Future<GeneralResponse<UserModel>> login(BuildContext context, dynamic body)async{
  ...
  Future<GeneralResponse> createAccount(BuildContext context, dynamic body)async{
    ...
    try {
      ...
      if(!response.error){
        ...
        return GeneralResponse(message: response.message, error: false, );
      }

      return GeneralResponse(message: response.message, error: response.error);
    } catch (e) {
      GlobalHelper.logger.w("erro: $e");
      return GeneralResponse(message: 'Errro al Registrar usuario', error: true);
    }
  }
